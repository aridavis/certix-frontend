...
function Concert({ match }) {
  ...
  useEffect(() => {
    Con.Show(match.params.id).then((res) => {
      if (!res.data) {
        history.push("/");
      }
      setConcert(res.data);
        ...
        <>
          ...
          <Box className={classes.detailContainer} mx={20} my={5}>
            ...
            <Box className={classes.innerContainer}>
              <div style={{ flex: 2 }}>
                <Typography variant="h5" className={classes.concertDetail}>
                  <EventIcon className={classes.icon} /> {concert.start_time}
                </Typography>
                <Typography variant="h5" className={classes.concertDetail}>
                  <AlbumIcon className={classes.icon} /> {concert.genre.name}
                </Typography>
              </div>
              <div style={{ flex: 2 }}>
                <Typography variant="h5" className={classes.concertDetail}>
                  Individual: Rp. {concert.price}
                </Typography>
                <Typography variant="h5" className={classes.concertDetail}>
                  Quantity:{" "}
                  <RemoveCircleIcon
              ...
              <div style={{ flex: 2 }}>
                ...
                <Typography
                  ...
                  className={classes.concertDetail}
                >
                  {error}
                </Typography>
                <Typography
              ...
              >
                <Button
                  variant="contained"
                  color="primary"
                  style={{ color: "white", padding: "15px 30px" }}
                  size="large"
                  onClick={buyTicket}
                >
                  Buy Ticket
